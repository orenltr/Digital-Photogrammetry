...
def sampleGroundTruth(img):

    # sampling object and background pixels

    plt.title('sample 50 points of the object', fontweight="bold")
    plt.imshow(img2)
    object_points = plt.ginput(50)
    object_points = np.array(object_points, dtype=int)
    print(object_points)
    plt.close()

    plt.title('sample 50 points of the background', fontweight="bold")
    plt.imshow(img2)
    background_points = plt.ginput(50)
...
def generateGaborFiltersBank():
    ...
    for theta in range(4):  # define number of thetas (orientation of the filter)
        theta = theta / 4 * np.pi
        for sigma in (1, 3):  # define number of sigmas (std of the gaussian envelope)
